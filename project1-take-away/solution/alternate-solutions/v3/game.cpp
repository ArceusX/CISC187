...
void init_players(mesa::option o) {
    if (o.has_ai) {
...
void init_heaps(mesa::option o) {
    static std::random_device rd;
    static std::mt19937 gen(rd());
    static std::uniform_int_distribution<> piles(3, 10);
    static std::uniform_int_distribution<> items(1, 15);

    if (o.piles && o.stones) {
        for (auto i = 0u; i < o.piles; ++i) {
            game.stones.push_back(o.stones);
        }
    } else if (o.piles) {
        for (auto i = 0u; i < o.piles; ++i) {
            game.stones.push_back(items(gen));
        }
    } else if (o.stones) {
        for (auto i = 0; i < piles(gen); ++i) {
            game.stones.push_back(o.stones);
        }
    } else if (o.is_random) {
        for (auto i = 0; i < piles(gen); ++i) {
        ...
        auto heaps = get_val("Enter number of heaps: ");
        for (auto i = 0u; i < heaps; ++i) {
            game.stones.push_back(i+1);
        }
    }
}

void play(mesa::option o) {
    init_heaps(o);
